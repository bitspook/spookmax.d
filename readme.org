...
- Utilities for helping with scratch buffer
  ...
  #+begin_src elisp
    ...
    org-roam-dailies, capturing the \"reading\" template."
      (interactive)
      (let ((lower-window (split-window-below (or baby-size -20)))
            (window-name (or (when (boundp 'exwm-class-name) exwm-class-name) "")))
        (select-window lower-window)
        (cond
         ((string= window-name "Firefox")
          (org-roam-dailies--capture (current-time) t "r"))
...
- org-roam
  ...
  - Setup =org-roam=
    ...
    - Templates for capturing dailies. I've decided to give a shot to use
      ...
      EXWM. My primary use-case is to take quick notes about what I am reading
      right now, which is usually in Firefox. EXWM can access the window title,
      but not the URL of current tab. The URL is prompted right now, but I'll
      probably create a browser extension to communicate with Emacs when this
      gets annoying enough and I have some time to spare.

      #+begin_src elisp
        (defun spook--exwm-note-template ()
          (let ((window (or (when (boundp 'exwm-class-name) exwm-class-name) ""))
                (title (or (when (boundp 'exwm-title) exwm-title) "%^{TITLE}")))
            (cond
             ;; Putting this in here as an example if title need be extracted in a
             ;; different way for different windows.
