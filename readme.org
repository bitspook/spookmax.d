...
- [Ma]git
  ...
  Need to install llama manually because elpaca keeps trying to install it from sourcehut, where its
  repo don't exist.
  #+begin_src elisp
    (use-package llama
      :ensure (llama :host github :repo "tarsius/llama" :branch "main"))
  #+end_src
...
#+begin_src elisp
  (use-package plz
    :ensure (plz :fetcher github :repo "alphapapa/plz.el"))
#+end_src
