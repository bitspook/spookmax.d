...
- Utilities for helping with scratch buffer
  ...
  #+begin_src elisp
    ...
    (defun spook--scratch-below (&optional scratch-size)
      "Create a small window below with *scratch* as its buffer for quick access"
      (interactive)
      (let ((lower-window (split-window-below (or scratch-size -20))))
    ...
    - Templates for capturing dailies. I've decided to give a shot to use
      ...
      #+begin_src elisp
        (defun spook--exwm-note-template ()
          (let ((window (or exwm-class-name ""))
                (title (or exwm-title "%^{TITLE}")))
            (cond
