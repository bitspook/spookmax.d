...
- [Ma]git
  ...
  Magit uses =project-switch-commands= which are present only in more recent
  project.el project.
    ...
    (use-package org-super-agenda
      ...
      (setq org-super-agenda-groups
            '((:name "In Progress" :todo "DOING" :order 1)
              ...
              (:name "Study 📚" :tag ("study" "reading") :order 4)
...
- Flycheck for getting those in-buffer warnings errors.
  ...
  #+begin_src elisp
    (let ((feglot-dir (expand-file-name "~/code/emacs/flycheck-eglot")))
      (when (file-exists-p feglot-dir)
        (add-to-list 'load-path feglot-dir)
...
- Completions
  ...
  #+begin_src elisp
    (let ((pkg-dir (expand-file-name "~/code/emacs/nerd-icons-corfu")))
...
- Spell checking
  ...
  #+begin_src elisp
    ...
    (define-key my-apps-keymap (kbd "s")
                (define-keymap
                  :parent my-apps-keymap
                  :name "spellcheck"
                  (kbd ".") #'flyspell-correct-at-point
                  (kbd ",") #'flyspell-correct-before-point))
  #+end_src  
...
- Libretranslate for translating Deutsch from Emacs
  ...
  #+begin_src elisp
    (let ((libretrans-dir (expand-file-name "~/code/emacs/libretrans.el")))
      (when (file-exists-p libretrans-dir)
        (add-to-list 'load-path libretrans-dir)
        (require 'libretrans)
...
- Direnv is pretty essential for my dev workflow.
  ...
  #+begin_src elisp
    ...
    ;; for emacs libraries I use within dev-envs created with guix
    (add-hook 'envrc-mode-hook
              (lambda ()
                (setq-local load-path load-path)
                (when (getenv "EMACSLOADPATH")
                  (let ((paths (split-string (getenv "EMACSLOADPATH") ":" t)))
...
#+begin_src elisp
  (message "Took %s to start Emacs" (emacs-init-time))
#+end_src
